...
> [!NOTE]
> 当前版本号规范使用点分隔预发布序号（如 `0.16.3-beta.1`）。仓库中存在连字符分隔的历史 tag（如 `0.15.2-beta-1`）以及带 `v` 前缀的早期 tag（如 `v0.1.0-beta`）。点/连字符两种格式在判定级别时视为等价，`git-tags.sh` 对两者均可识别与排序；完全不符合规范的历史 tag 按创建时间倒序排在标准 tag 之后。

## 判定规则

1. 如果用户未说明，假设当前目标版本为正式版本（Release）。
2. 分页调用 `./.agents/skills/release-note-creator/scripts/git-tags.sh --pagesize <N> --offset <M>` 获取历史 tag 列表。
...
7. 如果翻遍所有 tag 仍未找到符合条件的历史 tag，则变更范围为“仓库初始提交至当前目标 tag 的全部变化”，此时省略 Full Changelog 的 compare 链接。
...
流程：
1. 选定为正式版本（Release）
2. 分页调用 `./.agents/skills/release-note-creator/scripts/git-tags.sh` 获取历史 tag 列表
