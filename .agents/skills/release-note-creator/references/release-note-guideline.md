...
> 本规范旨在统一 Xiaomi Album Syncer 的发布说明（Release Notes）书写风格，
...
Full Changelog: https://github.com/Coooolfan/XiaomiAlbumSyncer/compare/<prev>...<curr>
```
...
| 分组              | 内容范围                 | 示例                             |
| :---------------- | :----------------------- | :------------------------------- |
| `## feat`         | 新增功能、改进、性能优化 | 新增录音同步；支持原生镜像构建   |
| `## fix`          | Bug 修复、稳定性修正     | 修复相册时间线偏移问题           |
| `## chore / docs` | 构建、依赖、文档、脚本   | 更新 README；调整 CI 缓存策略    |
| （可选）`## perf` | 性能优化（如需独立说明） | 优化增量同步扫描速度             |
...
| 子工程   | 涉及特定子工程时在描述中自然带出     | “服务端优化下载队列”、“Web 端修复设置页”  |

> 面向读者的称谓：`server` → 服务端；`web` → Web 端；`xas-mock` → Mock 服务。
> 不使用 commit scope（`server` / `web`）作为条目前缀。
...
## fix
* 修复相册增量同步时间戳比较错误 https://github.com/Coooolfan/XiaomiAlbumSyncer/commit/bc3bc75eb8750f73a30c0597a1746d80815f66c9
