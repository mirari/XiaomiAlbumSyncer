...
- 完整分页、相册数量一致性、下载 SHA1/大小及本地文件状态检查通过后才清理。
  认证失败、接口缺字段、分页异常、同名冲突、扫描期间云端变化均停止清理和基线提交。
- 每次只读取一轮完整云端文件快照，删除/移动旧路径在本次成功执行中处理，不等待下一轮确认。快照完成后的云端变化由下次同步发现。旧内容放入独立 state 下的
...
- 日常同步信任已建立的本地基线：云端 ID、路径和 SHA1 未变且已有本地校验记录时，
  不读取该文件的属性或内容，也不重复检查其目录。报告 `transfer.trusted` 记录跳过数量。
  云端有变化或缺少校验记录的文件仍执行路径安全检查、内容验证和隔离保护。
- 日常同步不发现本地单独修改、删除或磁盘损坏。独立执行模块使用 `--verify-local --apply`
  可强制重新读取全部本地内容校验，并补回缺失文件；本地内容冲突会停止执行，不静默覆盖。
  完整应用工作进程支持输入配置 `verify_local: true`；当前网页未提供完整校验按钮，需维护时手动执行。
